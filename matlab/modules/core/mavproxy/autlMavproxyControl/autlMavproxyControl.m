...
%   result     : struct with status, output, error_message
    ...
    if ~isfield(cfg, 'mav') || ~isfield(cfg.mav, 'master_connection')
        % Use the single-UAV SITL heartbeat endpoint by default.
    ...
    if allow_port_fallback
        if contains(master_conn, ':14550')
            master_candidates{end+1} = 'tcp:127.0.0.1:5760';
            master_candidates{end+1} = 'tcp:127.0.0.1:5762';
        elseif contains(master_conn, ':5760')
            master_candidates{end+1} = 'tcp:127.0.0.1:5762';
            master_candidates{end+1} = 'udpin:127.0.0.1:14550';
    ...
    function [ok, out, err] = run_pymav(action_name, params_struct)
        ...
        py_template = [ ...
            ...
            '  def enum_name(enum_group, value):' newline ...
            '    table = getattr(mavutil.mavlink, "enums", {}).get(enum_group, {})' newline ...
            '    entry = table.get(int(value))' newline ...
            '    return getattr(entry, "name", str(int(value))) if entry is not None else str(int(value))' newline ...
            '  def heartbeat_summary():' newline ...
            '    return (' newline ...
            '      f"system_id={m.target_system} component_id={m.target_component} "' newline ...
            '      f"type={enum_name(''MAV_TYPE'', getattr(hb, ''type'', -1))} "' newline ...
            '      f"autopilot={enum_name(''MAV_AUTOPILOT'', getattr(hb, ''autopilot'', -1))} "' newline ...
            '      f"base_mode={int(getattr(hb, ''base_mode'', 0))} system_status={int(getattr(hb, ''system_status'', 0))} "' newline ...
            '      f"custom_mode={int(getattr(hb, ''custom_mode'', 0))}"' newline ...
            '    )' newline ...
            '  def ensure_vehicle_heartbeat():' newline ...
            '    autopilot = int(getattr(hb, "autopilot", -1))' newline ...
            '    if autopilot == int(getattr(mavutil.mavlink, "MAV_AUTOPILOT_INVALID", -1)):' newline ...
            '      raise RuntimeError(f"non-vehicle heartbeat: {heartbeat_summary()}")' newline ...
            ...
            '    ensure_vehicle_heartbeat()' newline ...
            ...
            '    ensure_vehicle_heartbeat()' newline ...
            '    print(heartbeat_summary())' newline ...
            '    pass' newline ...
            '  else:' newline ...
...
if rc == 0
    ...
    if strcmpi(action, 'set_mode') && service_ok && isfield(params, 'mode')
        [confirm_ok, confirm_out] = autlWaitForMavrosMode(ns, source_overlay, shell_timeout_s, char(string(params.mode)));
        if ~confirm_ok
            [mode_ok, mode_out] = autlTryMavrosForceSetMode(ns, source_overlay, shell_timeout_s, char(string(params.mode)));
            out = sprintf('%s\n[mode_confirm_failed]\n%s', char(string(out)), char(string(confirm_out)));
            result.output = sprintf('%s\n[mode_confirm_failed]\n%s', char(string(result.output)), char(string(confirm_out)));
            result.output = sprintf('%s\n[force_mode_fallback]\n%s', char(string(result.output)), char(string(mode_out)));
            if mode_ok
                service_ok = true;
            else
...
function [ok, out] = autlWaitForMavrosMode(ns, source_overlay, timeout_s, expected_mode)
% Wait until /mavros/state reports the requested mode.

ok = false;
out = '';

expected_mode_u = upper(strtrim(char(string(expected_mode))));
poll_s = 0.4;
start_t = tic;
while toc(start_t) < timeout_s
    remain_s = max(1, ceil(timeout_s - toc(start_t)));
    sample_timeout_s = min(3, remain_s);
    cmd = sprintf(['bash -lc ''set +u; source /opt/ros/humble/setup.bash >/dev/null 2>&1; %sset -u; timeout %d ' ...
        'ros2 topic echo --qos-reliability best_effort --qos-durability volatile --once %s/state'''], ...
        source_overlay, sample_timeout_s, ns);
    [rc, cmd_out] = system(cmd);
    out = cmd_out;

    if rc == 0
        out_u = upper(char(string(cmd_out)));
        mode_match = regexp(out_u, '(?m)^\s*MODE:\s*([A-Z0-9_]+)\s*$', 'tokens', 'once');
        if isempty(mode_match)
            mode_match = regexp(out_u, '(?m)^\s*CUSTOM_MODE:\s*([A-Z0-9_]+)\s*$', 'tokens', 'once');
        end
        if ~isempty(mode_match) && strcmp(strtrim(char(string(mode_match{1}))), expected_mode_u)
            ok = true;
            return;
        end
    end
